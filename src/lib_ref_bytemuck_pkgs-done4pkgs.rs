...
impl MetaFile {
    ...
    pub fn new(buf: &mut Vec<u8>, key: &[u8; 8]) -> Result<Self, Box<dyn Error>> {
        ...
        let package_table: Vec<PackageRecord> = pod_collect_to_vec(&mut reader.get_mut()[range]);

        let range = block_range(BlockType::Metas, &mut reader)?;
        let mut meta_table: Vec<MetaRecord> = pod_collect_to_vec(&mut reader.get_mut()[range]);
